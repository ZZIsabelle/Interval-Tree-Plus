...
"ivl_tree ≡ Node Leaf ((Abs_ivl (0::nat, 5::nat),5::nat),Black) Leaf"
...
lemma"insert_IntervalT nat_ivl ivl_tree = ivl_tree"
  apply(simp add:insert_IntervalT_def nat_ivl_def ivl_tree_def max3_def high_def)
  by(simp add: Abs_ivl_inverse Rep_ivl_inverse)

lemma"insert_IntervalT (Abs_ivl (0::nat, 5::nat)) Leaf = ivl_tree"
  apply(simp add:insert_IntervalT_def max3_def high_def ivl_tree_def)
  by(simp add: Abs_ivl_inverse Rep_ivl_inverse)
...
lemma test3:"(X_P_Achievements CS ⟨⟩) = 
⟨⟨⟩,((Abs_ivl (5::nat,12::nat),28::nat),Black),⟨⟨⟩,((Abs_ivl (16::nat,28::nat),28::nat),Red),⟨⟩⟩⟩"
  apply(simp add:CS_def X_P_Achievements_def X_Dimension_def map_Publication_def insert_IntervalT_def max3_def high_def)
  by(simp add: Abs_ivl_inverse ivl_less low_def high_def)

lemma test4:"(Y_P_Achievements CS ⟨⟩) = 
⟨⟨⟩,((Abs_ivl (10::nat,22::nat),22::nat),Black),⟨⟨⟩,((Abs_ivl (14::nat,20::nat),20::nat),Red),⟨⟩⟩⟩"
  apply(simp add:CS_def Y_P_Achievements_def Y_Dimension_def map_Publication_def insert_IntervalT_def max3_def)
  by(simp add: Abs_ivl_inverse ivl_less low_def high_def)


lemma test5:"Match CS Leaf (Abs_ivl (14::nat,21::nat),Abs_ivl (6::nat,16::nat)) = True"
  apply(simp add:Match_def judge_def CS_def Y_P_Achievements_def Y_Dimension_def map_Publication_def insert_IntervalT_def X_P_Achievements_def X_Dimension_def)
